...
test('释放拖拽后惯性按真实时间衰减并收敛停止，收敛后无输入相机不再漂移（SPEC-7.1 + SPEC-7.5）', async ({
  ...
  // 地球本体未被拖拽/惯性带动（SPEC-7.1：作用于相机，地球本体不因拖拽转动）。
  // 断言取 t2（累计 ~4.2s，惯性早已衰减完、拖拽两阶段均已完成）而非文末 t5（累计 ~8.2s）：
  // BUG-010——8-worker 负载下 setup/往返叠加会把 t5 的空闲计时（自 mouse.up 起算）推逼近
  // SPEC-7.3 的 10s 阈值，一旦触发**空闲自转**（与本场景 SPEC-7.1「拖拽/惯性不转动地球本体」
  // 正交的另一机制）就会改写 earthRotY、令该 10 位精度断言偶发红。t2 距 10s 阈值有充分余量
  // （空闲计时自 mouse.up 起算，与 setup 耗时无关），且拖拽 + 惯性到 t2 已全部完成，故
  // t2.earthRotY==before 已充分证明二者不转动地球本体——SPEC-7.1 判据完整保留、无覆盖损失。
  expect(t2.earthRotY).toBeCloseTo(before.earthRotY, 10)

  // 收敛后继续静置，确认无任何输入时相机位姿不再变化（不漂移）——
  // 若此处观察到持续漂移，即违反 SPEC-7.1 的收敛判据。相机方位角（azimuthOf 读相机 x/z）
  // 不受空闲自转影响（自转只转地球本体、不动相机），故此墙钟末端的漂移检查对负载稳健。
  await page.waitForTimeout(1000)
