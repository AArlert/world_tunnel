...
//         仍保持单层 InstancedMesh（光柱，子节点数恒 1，DP M3-marker-pillar §3.2 切片①
//         精确值），不整表重建为逐事件 Mesh；
...
//
// 可见状态量测：以标记层唯一 InstancedMesh（光柱，children[0]）的 per-instance
// instanceAlpha（呼吸 alpha 观测通道；DP M3-marker-pillar §3.1 (a)(b)(c) 三条不变式保证
// 该通道即驱动渲染可见度的呼吸透明度）逐帧采样为确定性主证据，辅以分类色像素查找
// （findColorInRegion）与截图作为「视觉」佐证。标记实例按 lat/lon 经 SPEC-6.2 换算得的
// 方向识别（不依赖实现内部槽位次序）。
...
// severity 3：柱高最高（SPEC-3.7a）、分级色不打折（SPEC-3.7 sev3=分类 pin 色本身），
// 光柱色像素足够大，findColorInRegion 命中数稳定。
...
test('呼吸式过渡：旧标记渐隐 / 新标记渐亮 / 既有标记连续可见（SPEC-3.11，多帧采样 + 截图）', async ({
  ...
  // 1 层 InstancedMesh（光柱；辉光/足印改 shader 内梯度，不再有独立 rings mesh，DP
  // M3-marker-pillar §3.2 逐切片钉精确子节点数：切片①=1），不逐事件建 Mesh、不整表重建。
  expect(await markerGroupChildCount(page)).toBe(1)
