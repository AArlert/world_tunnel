...
test('呼吸式过渡：旧标记渐隐 / 新标记渐亮 / 既有标记连续可见（SPEC-3.11，多帧采样 + 截图）', async ({
  ...
  await resetIdleTimer(page)
  const t0 = Date.now()
  await setDebugEvents(page, [mk(KEEP, 'keep'), mk(NEW, 'new')])

  // 连续多帧采样（非仅前/后两张静态截图）：紧接注入后开始，逐帧读取三枚标记 alpha。
  const samples: { t: number; keep: number; old: number; new: number }[] = []
  let midEarlyShot = false
  let midLateShot = false
  for (let i = 0; i < 20; i++) {
    const inst = await sampleMarkerInstances(page)
    const rec = {
      t: Date.now() - t0,
      keep: alphaAt(inst, dirKeep),
      old: alphaAt(inst, dirOld),
      new: alphaAt(inst, dirNew),
    }
    samples.push(rec)
    // 过渡中途留两帧截图（视觉「连续多帧」证据）：一早一晚各一张部分过渡态。
    if (!midEarlyShot && rec.old < 0.8 && rec.old > 0.55) {
      await page.screenshot({ path: 'test-results/m2-21-mid-early.png' })
      midEarlyShot = true
    }
    if (!midLateShot && rec.old < 0.45 && rec.old > 0.15) {
