...
| FM-05 | M2 | 数据核心 | src/data/（GeoEvent/scheduler/缓存/去重/过期；SPEC-5.0, 6.1, 6.2, 6.3） | （M2 开卡登记） |
...
| FM-10 | M2 | 顶栏 + 基础分类过滤 | src/ui/（品牌名 Worlens · UTC 时钟 · 六 category 开关；SPEC-2.1, 2.4①, 8.1 分类子集） | （M2 开卡登记） |
| FM-11 | M2 | 性能预算基线 | 首包/冷启动基线量测（SPEC-3.10 基线, 3.8） | （M2 开卡登记） |
| FM-12 | M3 | 扩展信源 | providers/（GDELT/OpenSky/CoinGecko；SPEC-5.4, 5.6, 5.7） | （M3 开卡登记） |
...
非路线图流程 gate（orch 台账跟踪）：D2 营销、D4 商标检查、D5 用户验证、D16 阶段二触发、D19 license+CLA（详见 doc/product-decisions.md）。
