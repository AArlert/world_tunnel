...
| M2-01 | M2 | GeoEvent 归一化模型字段与坐标一致性（单测）：任意一条归一化后的 GeoEvent 须满足 SPEC-6.1 定义的字段与类型约束——`id` 形如 `{source}:{原始id}` 且全局唯一；`category` 属六值枚举之一；`severity`∈{1,2,3}；`urls` 为数组，除 flight 外长度 ≥1；`lat`/`lon` 为 WGS84 度数、`ts` 为 epoch ms、`source` 属枚举字符串集合；由其 lat/lon 生成的三维坐标须遵循 SPEC-6.2 约定（与 `geo.latLonToVector3` 同一约定：北极 (90,·)→+Y，(0,0)→+Z，(0,90°E)→+X） | 🔲 |  |  |
| M2-02 | M2 | 同 id 更新去重与默认过期窗（单测）：同一 `id` 的事件再次出现时覆盖 ts/severity/summary 且不产生新标记（去重键为 id，SPEC-6.3 第一句）；未再出现更新的事件在 48–72h 区间内的某过期窗后被移除（具体值可配，不预设精确数字，SPEC-6.3①）。SPEC-6.3① flight-60s 子句不在本场景：M2 无 opensky 源，依 REV-007 F-1 裁决改挂 M3 FM-12（opensky provider 场景）承接，见 BUG-016 | ✅ | doc/evidence/v0.2.0/M2-02.log | `make test TEST=store` |
