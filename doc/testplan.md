...
| M2-02 | M2 | 同 id 更新去重与默认过期窗（单测）：同一 `id` 的事件再次出现时覆盖 ts/severity/summary 且不产生新标记（去重键为 id，SPEC-6.3 第一句）；未再出现更新的事件在 48–72h 区间内的某过期窗后被移除（具体值可配，不预设精确数字，SPEC-6.3①）。SPEC-6.3① flight-60s 子句不在本场景：M2 无 opensky 源，依 REV-007 F-1 裁决改挂 M3 FM-12（opensky provider 场景）承接，见 BUG-016 | 🔲 |  |  |
| M2-03 | M2 | 独立轮询与限流预算（单测）：不同数据源的调度按各自独立的轮询周期触发，一源的轮询节奏或限流预算不与另一源共享或相互干扰（SPEC-5.0「每源独立轮询与限流预算」） | 🔲 |  |  |
| M2-04 | M2 | HTTP 失败退避、条件请求与故障隔离（单测）：单源连续请求失败时重试间隔按「基础间隔×2^n」指数增长且不超过 30 分钟上限（SPEC-5.0 退避条款）；对声明支持 ETag/Last-Modified 的源发起请求时附带对应的条件请求头（SPEC-5.0 条件请求条款）；任一源持续故障（如网络错误/5xx）不得导致其他源的轮询停止、也不得影响渲染主循环（SPEC-5.0 故障隔离条款） | 🔲 |  |  |
